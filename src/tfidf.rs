...
pub(crate) fn document_path_to_content(
...
#[allow(dead_code)]
pub(crate) fn get_document_paths_in_directory(dir_path: &str) -> Vec<String> {
...
pub(crate) fn document_to_count(
...
pub(crate) type DocToTermCountMap = HashMap<Document, HashMap<String, usize>>;
pub(crate) type TermToDocCountMap = HashMap<String, HashMap<Document, usize>>;

pub(crate) fn document_and_term_to_count(
    document_to_content: &HashMap<Document, Option<DocumentContent>>,
...
pub(crate) fn document_to_term_to_tf(
    document_to_term_to_count_map: &HashMap<Document, HashMap<String, usize>>,
...
pub(crate) fn document_to_sorted_terms(
...
fn document_to_term_to_tf_idf_map(
    ...
    document_to_term_to_tf_map
        ...
        .map(|(document, term_to_tf_map)| {
            let term_to_tf_idf_map: HashMap<String, f64> = term_to_tf_map
                .iter()
                .map(|(term, tf)| {
                ...
                .collect();
            (document.clone(), term_to_tf_idf_map)
        })
        .collect()
}

fn term_to_document_to_tf_idf_map(
    document_to_term_to_tf_map: &HashMap<Document, HashMap<String, f64>>,
    document_to_term_to_idf_map: &HashMap<Document, HashMap<String, f64>>,
) -> HashMap<String, HashMap<Document, f64>> {
    let mut term_to_document_to_tf_idf_map: HashMap<String, HashMap<Document, f64>> =
        HashMap::new();

    for (document, term_to_tf_map) in document_to_term_to_tf_map.iter() {
        for (term, tf) in term_to_tf_map.iter() {
            let idf = document_to_term_to_idf_map
                .get(document)
                .and_then(|term_to_idf_map| term_to_idf_map.get(term))
                .unwrap_or(&0.0);
            term_to_document_to_tf_idf_map
                .entry(term.to_owned())
                .or_default()
                .insert(document.clone(), tf * idf);
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_term_to_document_to_tf_idf_map() {
        let content_1: DocumentContent = DocumentContent {
            content: "the cats are in the house".to_string(),
        };
        let content_2: DocumentContent = DocumentContent {
        ...
        // when:
        let actual_term_to_document_to_tf_idf_map: HashMap<String, HashMap<Document, f64>> =
            term_to_document_to_tf_idf_map(
                &document_to_term_to_tf_map,
                &document_to_term_to_idf_map,
            );

        let expected_term_to_document_to_tf_idf_map: HashMap<&str, HashMap<Document, f64>> = hashmap![
            "the" => hashmap![
        ...
        for (expected_term, expected_doc_to_tf_idf_map) in
            expected_term_to_document_to_tf_idf_map.iter()
        {
            let actual_doc_to_tf_idf_map = actual_term_to_document_to_tf_idf_map
                .get(expected_term.to_owned())
                .unwrap();

            for (expected_doc, expected_tf_idf) in expected_doc_to_tf_idf_map.iter() {
                let actual_tf_idf: &f64 = actual_doc_to_tf_idf_map.get(expected_doc).unwrap();
                assert_eq!(actual_tf_idf, expected_tf_idf);
            }
        }
